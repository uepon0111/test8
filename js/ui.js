...
import { DEFAULT_DEVICE_KEY, buildDeviceProfile, detectDeviceKeyFromImage, ensureDeviceProfile, ensureSettingsShape, getDeviceLabel, listDeviceProfiles } from "./device.js";
...
function getActiveDeviceKey() {
  return state.settings?.activeDeviceKey || DEFAULT_DEVICE_KEY;
}

function getActiveProfile() {
  return ensureDeviceProfile(state.settings, getActiveDeviceKey());
}
...
function setActiveDeviceKey(deviceKey) {
  state.settings = ensureSettingsShape(state.settings);
  if (!state.settings.deviceProfiles[deviceKey]) {
    state.settings.deviceProfiles[deviceKey] = buildDeviceProfile(deviceKey);
  }
  state.settings.activeDeviceKey = deviceKey;
  state.currentDeviceKey = deviceKey;
}

function updateDeviceSelectOptions() {
  const profiles = listDeviceProfiles(state.settings);
  const activeKey = getActiveDeviceKey();
  const options = profiles.map((profile) => `<option value="${profile.key}">${escapeHtml(profile.label)}</option>`).join("");

  [el("settings-device-select"), el("up-device")].forEach((select) => {
    if (!select) return;
    const seen = new Set(profiles.map((p) => p.key));
    select.innerHTML = options;
    if (!seen.has(activeKey)) {
      select.insertAdjacentHTML("afterbegin", `<option value="${activeKey}">${escapeHtml(getDeviceLabel(activeKey))}</option>`);
    }
...
export function renderControlOptions() {
  ...
  if (sortSelect) {
    const currentSort = sortSelect.value || "level_desc";
...
export function renderGrid(records) {
  ...
  for (const rec of records) {
    const thumb = rec.thumbnail ? rec.thumbnail.replace("=s220", "=w320") : "";
    const large = rec.thumbnail ? rec.thumbnail.replace("=s220", "=w1400") : "";
    ...
    card.id = `card-${rec.id}`;
    card.onclick = state.isSelectMode ? () => toggleSelection(rec.id) : () => openImageModal(large);
    ...
    card.innerHTML = `
      <div class="card-img-container">
        ...
        ${thumb ? `<img data-src="${thumb}" class="card-img" loading="lazy" alt="${escapeHtml(rec.title)}">` : '<span style="color:#aaa;">NO IMAGE</span>'}
      </div>
      <div class="card-body">
        <div class="song-meta">
    ...
    if (img) {
      if (observer) observer.observe(img); else hydrateThumb(img);
      img.addEventListener("load", () => {
        img.style.opacity = 1;
        const spinner = img.parentElement?.querySelector(".img-loader-spinner");
...
function renderSettingsUI() {
  ...
  top.className = "crop-group";
  top.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap;">
      <div style="font-weight:700;">機種設定</div>
      <div class="settings-help">画像のサイズと比率から自動選択されます</div>
    </div>
  `;
  ...
  if (deviceSelect) {
    deviceSelect.onchange = (ev) => {
      setActiveDeviceKey(ev.target.value);
      renderSettingsUI();
    };
    deviceSelect.value = profile.key;
  }
...
function handleSettingsSampleUpload(ev) {
  ...
  reader.onload = () => {
    state.settings = ensureSettingsShape(state.settings);
    state.settings.activeDeviceKey = state.settings.activeDeviceKey || DEFAULT_DEVICE_KEY;
    const url = String(reader.result || "");
    const preview = el("settings-preview-img");
    if (preview) preview.src = url;
    const temp = new Image();
    temp.onload = () => {
      const detected = detectDeviceKeyFromImage(temp.naturalWidth, temp.naturalHeight, state.settings);
      setActiveDeviceKey(detected);
      const profile = getActiveProfile();
      profile.sampleImageDataUrl = url;
      profile.lastDetectedWidth = temp.naturalWidth;
      profile.lastDetectedHeight = temp.naturalHeight;
      profile.lastDetectedRatio = temp.naturalWidth / temp.naturalHeight;
      updateDeviceSelectOptions();
      const select = el("settings-device-select");
      if (select) select.value = detected;
      updateSettingsPreview();
      showToast("機種を自動判定しました", `${getDeviceLabel(detected)} として保存します。`, "success", 2200);
    };
    temp.src = url;
  };
...
function enableCropDragging() {
  ...
  if (!img || !layer) return;
  const boxes = [...layer.querySelectorAll(".crop-box")];
  boxes.forEach((box) => {
    ...
    box.onpointerdown = (ev) => {
      ...
      const start = getCropRegion(region);
...
function updateSettingsPreview() {
  ...
  regionKeys().forEach((key) => {
    const r = active.cropRegions?.[key] || DEFAULT_CROP_SETTINGS[key];
...
function resetSettings() {
  const profile = getActiveProfile();
  profile.cropRegions = JSON.parse(JSON.stringify(DEFAULT_CROP_SETTINGS));
...
async function detectDeviceKeyForUrl(url) {
  return await new Promise((resolve) => {
    const img = new Image();
    img.onload = () => resolve(detectDeviceKeyFromImage(img.naturalWidth, img.naturalHeight, state.settings));
    img.onerror = () => resolve(getActiveDeviceKey());
    img.src = url;
  });
}
...
export async function handleFiles(files) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const deviceKey = await detectDeviceKeyForUrl(imgUrl);
...
export function selectItem(id) {
  ...
  if (deviceSelect) {
    updateDeviceSelectOptions();
    if (!item.deviceKey) item.deviceKey = getActiveDeviceKey();
    deviceSelect.value = item.deviceKey || getActiveDeviceKey();
  }
...
export function updateCurrentItem(field, value) {
  ...
  if (field === "deviceKey") {
    item.deviceKey = value || getActiveDeviceKey();
    state.settings.activeDeviceKey = item.deviceKey;
    state.currentDeviceKey = item.deviceKey;
    updateDeviceSelectOptions();
  }
...
async function runBatchAnalysis(itemsToAnalyze) {
  ...
  for (const item of itemsToAnalyze) {
    ...
    try {
      ...
      if (!item.deviceKey) {
        item.deviceKey = detectDeviceKeyFromImage(img.naturalWidth, img.naturalHeight, state.settings);
      }
      const res = await analyzeLoadedImage(img, worker, item.deviceKey);
      if (res) {
        item.data = {
          title: res.title,
          level: res.level,
